...
#trains PermutoSDF to recover geometry as sdf+density and color given only posed images
# CALL with ./permuto_sdf_py/train_permuto_sdf_trans.py --dataset dtu --scene dtu_scan24 --comp_name comp_1 --exp_info default
...
def run_net(args, hyperparams, ray_origins, ray_dirs, img_indices, model_geo, model_rgb, model_bg, model_colorcal, occupancy_grid, iter_nr_for_anneal,  cos_anneal_ratio, forced_variance):
    with torch.set_grad_enabled(False):
        ray_points_entry, ray_t_entry, ray_points_exit, ray_t_exit, does_ray_intersect_box=model_geo.boundary_primitive.ray_intersection(ray_origins, ray_dirs)
        TIME_START("create_samples")
        fg_ray_samples_packed, bg_ray_samples_packed = create_samples(args, hyperparams, ray_origins, ray_dirs, model_geo.training, occupancy_grid, model_geo.boundary_primitive)
        
        if hyperparams.do_importance_sampling and fg_ray_samples_packed.samples_pos.shape[0]!=0:
            fg_ray_samples_packed=importance_sampling_sdf_model(model_geo, fg_ray_samples_packed, ray_origins, ray_dirs, ray_t_exit, iter_nr_for_anneal)
    ...
    if fg_ray_samples_packed.samples_pos.shape[0]==0: #if we actualyl have samples for this batch fo rays
        ...
        #foreground 
        #get sdf
        sdf, sdf_gradients, density, geom_feat=model_geo.get_sdf_density_and_gradient(fg_ray_samples_packed.samples_pos, iter_nr_for_anneal)
        #get rgb
        ...
        weights, weights_sum, bg_transmittance, inv_s = model_rgb.volume_renderer_trans_neus.compute_weights(fg_ray_samples_packed, sdf, sdf_gradients, density, cos_anneal_ratio, forced_variance) #neus
        pred_rgb=model_rgb.volume_renderer_trans_neus.integrate(fg_ray_samples_packed, rgb_samples, weights)

        #compute also normal by integrating the gradient
        grad_integrated_per_ray=model_rgb.volume_renderer_trans_neus.integrate(fg_ray_samples_packed, sdf_gradients, weights)
...
def run_net_in_chunks(frame, chunk_size, args, hyperparams, model_geo, model_rgb, model_bg, occupancy_grid, iter_nr_for_anneal, cos_anneal_ratio, forced_variance):
    ...
    for i in range(len(ray_origins_list)):
        ...
        #run net 
        pred_rgb, pred_rgb_bg, pred_normals, sdf_gradients, weights_sum, fg_ray_samples_packed  =run_net(args, hyperparams, ray_origins, ray_dirs, None, model_geo, model_rgb, model_bg, None, occupancy_grid, iter_nr_for_anneal, cos_anneal_ratio, forced_variance)
...
def run_net_sphere_traced(frame, args, hyperparams, model_geo, model_rgb, model_bg, occupancy_grid, iter_nr_for_anneal, cos_anneal_ratio, forced_variance,  nr_sphere_traces, sdf_multiplier, sdf_converged_tresh):
    ...
    ray_end_gradient_integrated=model_rgb.volume_renderer_trans_neus.integrate(traced_samples_packed, ray_end_gradient, weights)
    ...
    pred_rgb_integrated=model_rgb.volume_renderer_trans_neus.integrate(traced_samples_packed, rgb_samples, weights) 
...
def train(args, config_path, hyperparams, train_params, loader_train, experiment_name, with_viewer, checkpoint_path, tensor_reel, frames_train=None, hardcoded_cam_init=True):
    ...
    if hyperparams.use_occupancy_grid:
        ...
        occupancy_grid=None
    model_geo.train(phase.grad)
    model_rgb.train(phase.grad)
    model_bg.train(phase.grad)
    

    params=[]
    params.append( {'params': model_geo.parameters(), 'weight_decay': 0.0, 'lr': hyperparams.lr, 'name': "model_geo"} )
    ...
    while is_in_training_loop:
        ...
        if in_process_of_sphere_init:
            loss, loss_sdf, loss_eikonal= loss_sphere_init(args.dataset, 30000, aabb, model_geo, iter_nr_for_anneal )
            ...
            # if True:
                sdf_shifted, sdf_curvature=model_geo.get_sdf_and_curvature_1d_precomputed_gradient_normal_based( fg_ray_samples_packed.samples_pos, sdf_gradients, iter_nr_for_anneal)
            ...
            if hyperparams.use_occupancy_grid:
                ...
                offsurface_points=model_geo.boundary_primitive.rand_points_inside(nr_points=1024)
                sdf_rand, _=model_geo( offsurface_points, iter_nr_for_anneal)
            ...
            with torch.set_grad_enabled(False):
                ...
                if phase.iter_nr%8==0 and hyperparams.use_occupancy_grid:
                    ...
                    sdf_grid,_=model_geo( grid_centers_random, iter_nr_for_anneal) 
                    occupancy_grid.update_with_sdf_random_sample(grid_center_indices, sdf_grid, model_rgb.volume_renderer_trans_neus.get_last_inv_s(), 1e-4 )
                    # occupancy_grid.update_with_sdf_random_sample(grid_center_indices, sdf_grid, model_rgb.volume_renderer_trans_neus.get_last_inv_s().item(), 1e-4 )
                ...
                #increase also the WD on the encoding of the model_rgb to encourage the network to get high detail using the model_geo
                if iter_nr_for_anneal>=hyperparams.iter_start_reduce_curv:
        ...
        if train_params.save_checkpoint() and phase.iter_nr%10000==0:
            model_geo.save(checkpoint_path, experiment_name, phase.iter_nr)
            ...
            if hyperparams.use_occupancy_grid:
                path_to_save_model=model_geo.path_to_save_model(checkpoint_path, experiment_name, phase.iter_nr)
        ...
        if with_viewer and (phase.iter_nr%300==0 or phase.iter_nr==1 or ngp_gui.m_control_view):
            with torch.set_grad_enabled(False):
                ...
                if use_volumetric_render:
                    chunk_size=50*50
                    pred_rgb_img, pred_rgb_bg_img, pred_normals_img, pred_weights_sum_img=run_net_in_chunks(frame, chunk_size, args, hyperparams, model_geo, model_rgb, model_bg, occupancy_grid, iter_nr_for_anneal, cos_anneal_ratio, forced_variance)
                else:
                    pred_rgb_img, pred_rgb_bg_img, pred_normals_img, pred_weights_sum_img=run_net_sphere_traced(frame, args, hyperparams, model_geo, model_rgb, model_bg, occupancy_grid, iter_nr_for_anneal, cos_anneal_ratio, forced_variance,  nr_sphere_traces=15, sdf_multiplier=0.9, sdf_converged_tresh=0.0002)
                #vis normals
