...
class Texture(torch.nn.Module):
    ...
    def __init__(self, in_channels, boundary_primitive, geom_feat_size_in, nr_iters_for_c2f):
        ...
        mlp_in_channels=self.encoding.output_dims() + 25 + 3 + geom_feat_size_in
        self.mlp=LipshitzMLP(mlp_in_channels, [64,64,3], last_layer_linear=True)
...
class GeometryA(torch.nn.Module):
    ...
    def __init__(self, in_channels, boundary_primitive, geom_feat_size_out, nr_iters_for_c2f):
        ...
        self.sdf_shift=1e-2
        self.mlp_sdf_density= torch.nn.Sequential(
            torch.nn.Linear(self.encoding.output_dims(), 64),
            torch.nn.GELU(),
            torch.nn.Linear(64,2+geom_feat_size_out)
        )
        apply_weight_init_fn(self.mlp_sdf_density, leaky_relu_init, negative_slope=0.0) 
        leaky_relu_init(self.mlp_sdf_density[-1], negative_slope=1.0) 
        with torch.set_grad_enabled(False):
            self.mlp_sdf_density[-1].bias+=self.sdf_shift #faster if we just put it in the bias
    ...
    def forward(self, points, iter_nr):
        ...
        point_features=self.encoding(points, window.view(-1))
        sdf_density_and_feat=self.mlp_sdf_density(point_features) 
        
        density=sdf_density_and_feat[:,0:1]
        density=self.softplus(density) #similar to mipnerf
        sdf=sdf_density_and_feat[:,1:2] 

        return sdf, density, sdf_density_and_feat
...
class GeometryB(torch.nn.Module):

    def __init__(self, in_channels, boundary_primitive, geom_feat_size_out, nr_iters_for_c2f):
        super(GeometryB, self).__init__()
        ...
        self.mlp_sdf= torch.nn.Sequential(
            torch.nn.Linear(self.encoding.output_dims(), 64),
            torch.nn.GELU(),
            torch.nn.Linear(64,1+geom_feat_size_out)
        )
        self.mlp_density= torch.nn.Sequential(
            torch.nn.Linear(self.encoding.output_dims() + 1 + geom_feat_size_out, 64),
            torch.nn.GELU(),
            torch.nn.Linear(64,1+geom_feat_size_out)
        )
        apply_weight_init_fn(self.mlp_density, leaky_relu_init, negative_slope=0.0)
        apply_weight_init_fn(self.mlp_sdf, leaky_relu_init, negative_slope=0.0)
        leaky_relu_init(self.mlp_density[-1], negative_slope=1.0)
    ...
    def forward(self, points, iter_nr):
        ...
        point_features=self.encoding(points, window.view(-1))
        mlp_density_in = geom_feat=torch.cat([point_features,sdf_and_feat],1)
        density_and_feat=self.mlp_density(mlp_density_in)
        ...
        geom_feat=torch.cat([sdf,density_and_feat],1)
...
class GeometryC(torch.nn.Module):

    def __init__(self, in_channels, boundary_primitive, geom_feat_size_out, nr_iters_for_c2f):
        super(GeometryC, self).__init__()
    ...
    def forward(self, points, iter_nr):
        ...
        point_features=self.encoding(points, window.view(-1))
        density_and_feat=self.mlp_density(point_features)
        sdf_and_feat=self.mlp_sdf(point_features)
        ...
        geom_feat=torch.cat([density_and_feat,sdf_and_feat],1)
